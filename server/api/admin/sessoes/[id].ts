...
export default defineEventHandler(async (event) => {
  ...
  if (getMethod(event) === 'PUT') {
    ...
    const { nome_sessao, produto_tipo, pacote_index, fotos_incluidas, preco_foto_extra, status, prazo_selecao, valor_restante_pacote } = body ?? {};
    ...
    await svc.update(
      ...
      prazo_selecao ?? null,
      valor_restante_pacote !== undefined ? Number(valor_restante_pacote) : undefined,
    );
