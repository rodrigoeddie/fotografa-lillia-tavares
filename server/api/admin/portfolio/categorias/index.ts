...
/** GET /api/admin/portfolio/categorias — lista todas
 *  POST /api/admin/portfolio/categorias — cria nova */
export default defineEventHandler(async (event) => {
  await validateAdminToken(event);
  const svc = new CategoriasService(getOrm(event));

  if (getMethod(event) === 'GET') {
    return svc.listPortfolio();
  }
  ...
  if (getMethod(event) === 'POST') {
    ...
    const result = await svc.createPortfolio({
      slug: slug.trim(),
      titulo: titulo.trim(),
