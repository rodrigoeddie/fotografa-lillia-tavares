...
export default defineEventHandler(async (event) => {
  ...
  const workSlug = getRouterParam(event, 'work');
  if (!category || !workSlug) throw createError({ statusCode: 400, statusMessage: 'params obrigatórios' });

  const slug = `${category}/${workSlug}`;
  const svc = new PortfolioService(getOrm(event));
  const work = await svc.getBySlug(slug);
  if (!work || work.ativo === 0) {
